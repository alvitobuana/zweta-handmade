...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-4xl font-serif text-dark-brown font-bold mb-2">Tambah Produk</h1>
        ...
        <div id="top-action-buttons" class="flex items-center gap-3 transition-all duration-300">
            ...
            <button type="submit" form="create-product-form" class="px-6 py-2.5 bg-caramel text-white rounded-xl text-xs font-semibold hover:bg-opacity-95 transition shadow-sm">
                Simpan Produk
            </button>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-[0_4px_20px_rgba(28,20,16,0.01)]">
        <form id="create-product-form" action="{{ route('admin.products.store') }}" method="post" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-10">
            @csrf
            ...
            <div class="lg:col-span-5 flex flex-col">
                ...
                <div id="preview-image-container" class="w-full h-80 bg-[#FAF0E6] rounded-3xl flex flex-col items-center justify-center overflow-hidden mb-4 relative border border-2 border-dashed border-[#A56A43]/30 cursor-pointer hover:bg-[#FAF0E6]/80 transition text-center p-4">
                    ...
                    <span class="text-[10px] text-gray-400 mt-1">Drag & drop gambar ke sini atau klik untuk upload</span>
                </div>
            ...
            <div class="lg:col-span-7 flex flex-col gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400" value="{{ old('name') }}" placeholder="Tote Ferro" required>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel">
                            ...
                            <option value="custom" {{ old('status') == 'custom' ? 'selected' : '' }}>Custom</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" min="0" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400" value="{{ old('price') }}" placeholder="125000" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-dark-brown uppercase tracking-wider mb-2">Stok</label>
                        <input type="number" name="stock" min="0" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400" value="{{ old('stock') }}" placeholder="8">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="material" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400" value="{{ old('material') }}" placeholder="Rajut premium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-dark-brown uppercase tracking-wider mb-2">Ukuran</label>
                        <input type="text" name="size" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400" value="{{ old('size') }}" placeholder="25 x 20 x 8 cm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="colors" class="w-full px-4 py-3 bg-white border border-soft-beige rounded-xl text-sm focus:outline-none focus:border-caramel placeholder-gray-400 mb-3" value="{{ old('colors') }}" placeholder="Coklat, Beige, Pink, Sage">
